...
def text_w2v_test(text):
    length = len(text)
    f = codecs.open('Log/Word2VecTest.log', 'a', 'utf8')
    for i in xrange(length):
        print_flag = True;
        if isinstance(text[i], str):
            text[i] = text[i].decode('utf8')
        for word in word_tokenize(text[i]):
            if word not in w2v.vocab and word not in exclude:
                if print_flag:
                    f.write('\nSentence ' + str(i) + ':\n')
                    print_flag = False
                f.write(word + ' ')
...
text_w2v_test(knowledge_base_text)
text_w2v_test(question_text)
...
Target_Answer   = T.argmax(Target_Matrix, axis=1)
Equal_Vector    = T.eq(Predict_Answer, Target_Answer)
Right_Num       = T.sum(Equal_Vector)
Length          = Target_Matrix.shape[0]
Right_Percentage= T.cast(Right_Num, theano.config.floatX) / Length
...
debug_model = theano.function(
         inputs=[index],
         outputs=[Cost, Result, Target_Matrix, Predict_Answer,
                  Target_Answer, Equal_Vector, Right_Num, Length, Right_Percentage],
         givens={
...
while (epoch < n_epoch) and (not done_looping):
    epoch = epoch + 1
    training_result = [train_model(i)
                     for i in xrange(n_train_batches)]
    training_losses = [training_result[i][0]
                       for i in xrange(n_train_batches)]
    training_percent = [training_result[i][1]
                        for i in xrange(n_train_batches)]
    mean_training_loss = np.mean(training_losses)
    mean_training_percent = np.mean(training_percent)

    print(
            'epoch %i, minibatch %i, train loss %f, right percent %.2f' %
            (
                epoch,
                n_train_batches,
                mean_training_loss,
                mean_training_percent
            )
         )


    validation_result  = [valid_model(i)
                          for i in xrange(n_valid_batches)]
    validation_losses  = [validation_result[i][0]
                          for i in xrange(n_valid_batches)]
    validation_percent = [validation_result[i][1]
                          for i in xrange(n_valid_batches)]
    mean_validation_loss = np.mean(validation_losses)
    mean_validation_percent = np.mean(validation_percent)
    print(
        '    validation loss %f, right percent %.2f%%' %
        (
            mean_validation_loss,
            mean_validation_percent * 100
        )
    )
    ...
    mean_test_percent = np.mean(right_percent)

    print(
        (
        '    test loss %f, right percent %.2f%%'
        ) %
        (
            mean_test_loss,
            mean_test_percent * 100
        )
    )
...
while (epoch < n_epoch) and (not done_looping):
    ...
    for minibatch_index in xrange(n_train_batches):
        training_result = train_model(minibatch_index)
        # print(
        #         'epoch %i, minibatch %i/%i, train loss %f, right percent %f' %
        #         (
        #             epoch,
        #             minibatch_index + 1,
        #             n_train_batches,
        ...
        if (iter + 1) % validation_frequency == 0:
            print(
                    'epoch %i, minibatch %i, training loss %f, right percent %.2f%%' %
                    (
                        epoch,
                        n_train_batches,
                        training_result[0],
            ...
            mean_validation_percent = np.mean(validation_percent)
            print(
                'epoch %i, minibatch %i/%i, validation loss %f, right percent %.2f%%' %
                (
                    epoch,
                    minibatch_index + 1,
                    n_train_batches,
